...
public class OrderServiceImpl implements OrderService {
    ...
    public ResOrderDto createOrder(ReqOrderDto req, HttpServletRequest request) {
        ...
        CompletableFuture<Void> reserveFut = CompletableFuture
                .supplyAsync(() -> {
                    try {
                        itemClient.reserveStocks(userId, reserveStockDtos);
                    } catch (BusinessException ex) {
                        throw new CompletionException(new BusinessException(NOT_ENOUGH_STOCK));
                    } catch (Exception ex) {
                        log.warn("예외 발생 - 원인: {}", ex.getMessage());
        ...
        // 4. 모두 완료될 때까지 대기
        CompletableFuture.allOf(itemFut, reserveFut).join();

        // 5. 결과 꺼내기
        List<ItemResponse> items = itemFut.join();
    ...
    public void onPaymentSucceed(Long orderId) {
        OrderPaidInfo orderPaidInfo = orderRepository.findOrderPaidInfoByOrderId(orderId)
                .orElseThrow(() -> new BusinessException(NOT_FOUND_ORDER));
