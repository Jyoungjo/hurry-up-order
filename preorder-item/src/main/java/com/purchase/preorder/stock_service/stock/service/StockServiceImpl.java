...
public class StockServiceImpl implements StockService {
    ...
    public void reserveStocks(Long userId, List<ReqReserveStockDto> reserveStockDtos) {
        ...
        if (containsStockNotInitialized(result)) {
            ...
            // 재시도
            log.info("재고 미초기화로 인한 재시도");
            result = redisService.reserveStockAtomically(List.of(), args.toArray(new String[0]));
            if (isSuccess(result)) return;

            log.warn("DB 재고 없음");
            throw new BusinessException(ExceptionCode.NOT_ENOUGH_STOCK);
        } else if (containsInsufficientStockForItem(result)) {
            log.warn("재고 선점 시도했으나 가용 재고 부족으로 인한 예외 발생");
            throw new BusinessException(ExceptionCode.NOT_ENOUGH_STOCK);
        } else if (containsInvalidItemIdOrQtyAtIndex(result)) {
            log.warn("Invalid itemId or Qty");
    ...
    private boolean containsInsufficientStockForItem(List<Object> result) {
        return result.stream()
                .skip(1)
                .anyMatch(o -> o.toString().startsWith("Insufficient stock for item:"));
    }

    private boolean containsInvalidItemIdOrQtyAtIndex(List<Object> result) {
        return result.stream()
                .skip(1)
                .anyMatch(o -> o.toString().startsWith("Invalid itemId"));
    }
